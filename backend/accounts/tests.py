...
class AccountsAPITestCase(APITestCase):
    ...
    def test_auth_endpoints_except_register(self):
        ...
        res_login = self.client.post('/api/v1/auth/login/', login_payload, format='json')
        ...
        # 2. GET /api/v1/auth/me/ (AUTHENTICATED USER PROFILE)
        ...
        res_refresh = self.client.post('/api/v1/auth/refresh/', {"refresh": refresh_token}, format='json')
        ...
        print(f"    Payload Sent  : {{'refresh': '{refresh_token[:25]}...'}}")
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {access_token}")
        res_logout = self.client.post('/api/v1/auth/logout/', {"refresh": refresh_token}, format='json')
        ...
        print(f"\n[5] POST /api/v1/auth/refresh/ (Testing Revoked Token)")
        print(f"    Payload Sent  : {{'refresh': '{refresh_token[:25]}...'}}")
        self.client.credentials()
        res_revoked = self.client.post('/api/v1/auth/refresh/', {"refresh": refresh_token}, format='json')
        ...
        print("=" * 60)
        print(f"  1. Login   (/api/v1/auth/login/)   : Status {res_login.status_code} {'PASS' if res_login.status_code == status.HTTP_200_OK else 'FAIL'}")
        print(f"  2. Me      (/api/v1/auth/me/)      : Status {res_me.status_code} {'PASS' if res_me.status_code == status.HTTP_200_OK else 'FAIL'}")
        print(f"  3. Refresh (/api/v1/auth/refresh/) : Status {res_refresh.status_code} {'PASS' if res_refresh.status_code == status.HTTP_200_OK else 'FAIL'}")
        print(f"  4. Logout  (/api/v1/auth/logout/)  : Status {res_logout.status_code} {'PASS' if res_logout.status_code == status.HTTP_200_OK else 'FAIL'}")
        print(f"  5. Revoked (/api/v1/auth/refresh/) : Status {res_revoked.status_code} {'PASS' if res_revoked.status_code == status.HTTP_401_UNAUTHORIZED else 'FAIL'}")
        print("=" * 60 + "\n")
